...
resolver, local trace-fixture importer, local package generator, and evaluator.
A local evaluation uses valid fixtures, adversarial examples, a crosswalk
matrix, generated bundles, and generated packages. The current result is a
local, reproducible evidence chain, with limits: fixtures are local, external
services are not contacted, persistent identifiers are local names, and
community package validation is not claimed.
...
evidence shape and toolchain needed to review one self-improvement cycle.
...
transitions, repair planning, package metadata, and evaluation results into one
machine-readable review surface.

## Related Work

ASIEP is positioned beside, not above, existing work. Self-refinement and
release-engineering systems focus on how agents improve behavior. Trace systems
record execution. Provenance models express activities, agents, and entities.
Packaging approaches support exchange of research objects or digital objects.
AI governance frameworks describe accountability needs.

ASIEP targets the gap between these layers: a minimal evidence profile for an
agent self-improvement event, with explicit mappings and lossiness rather than
a claim that one upstream system already carries the whole accountability
contract. The related-work matrix in `manuscript/related_work_matrix.md` marks
items that need external citation hardening in M8.
...
evaluator, paper assets, and paper linter.
...
privacy and access policies explicit.
...
examples, mappings, and paper claim layer. This makes the repository usable by
agents that need to validate, repair, import, package, evaluate, or lint paper
claims.
...
be read as reproducible v0.1 checks rather than deployment evidence.
...
but it does not measure robustness against open-ended adversaries.
...
evidence across lifecycle, gate, artifact, repair, package, and evaluation
surfaces.

This crosswalk is a local minimal mapping. It is meant to make interoperability
claims inspectable, not to assert conformance to every external ecosystem.
...
The claim registry and evidence map extend this accountability discipline to
the paper itself. A paper claim is not accepted just because it is plausible; it
must point to repository evidence, a metric, a paper asset, or a limitation.
...
names. The privacy check is a policy sentinel and key scan, not a deployment DLP
system. Digest checking is local SHA-256 recomputation, not signature
verification. The corpus is small and synthetic. The profile is not proven
complete for every agent system.

These limits are acceptable for v0.1 because the intended contribution is a
minimal evidence profile and reproducible local chain, not an external service
integration or final standard.

## Standardization Path

The next research step is to harden external references and related work, then
test ASIEP against real trace exports and broader attack corpora. A standards
path should proceed through profile stabilization, independent implementation,
interop tests, and clearer mappings to provenance and packaging communities.
...
evaluation reports, and paper claims. The result is not a new self-improving
agent. It is a reviewable evidence contract that future agents can inspect,
